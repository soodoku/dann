...
function (train.data, test.data, 
    p = dim(x)[2], kmetric = max(0.2 * n, 50), epsilon.list = c(1, 
        0.5, 2, 5), plus = 1, ...) 
{
    min2d <- function (mat) 
    {
    ...
    while (newd < oldd) {
        ...
        junk <- dannsub(x, y, km = kmetric, k = 5, epsilon = epsilon.list, 
            fullw = FALSE, scalar = TRUE, dims = seq(ncol(x)), iter.sub = 1, 
            iter = 1, method = "dann")
        dd <- min2d(junk$error)
        oldd <- newd
    ...
    junk <- list(errors = apply(test.data$y != dann(x, xx, y = y, 
        kmetric = kmetric, epsilon = epsilon.list, cv = FALSE), 2, 
        sum))
    kk <- knn(x, xx, y, k = 5)
